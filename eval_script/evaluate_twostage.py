...
from model.caption_generator_new import CaptionGenerator
...
def eval(model_sl, model_cg, data_loader, logger, saver, params):
    ...
    for idx, batch_data in enumerate(data_loader):
        ...
        if params['batch_log_interval'] != -1 and idx % params['batch_log_interval'] == 0:
            logger.info('eval: batch[%04d/%04d], elapsed time=%0.4fs, avg generated_sent: %d',
...
def construct_model(params, saver, logger):
    if params['checkpoint_sl'] is None or params['checkpoint_cg'] is None:
        logger.error('checkpoints are required for evaluation')
        exit()
    state_dict_cg, _ = saver.load_model(params['checkpoint_cg'])
    state_dict_sl, params_sl = saver.load_model(params['checkpoint_sl'])

    params['anchor_list'] = params_sl['anchor_list']
